...
jon(R, C, D, result(A, S)):-
    (
        jon(R2, C2, D2, S),
        (
            (A = north, R is R2 - 1, not(R < 0), C is C2, empty(R, C, S), D is D2);
            (A = south, R is R2 + 1, rows(RMAX), not(R >= RMAX), C is C2, empty(R, C, S), D is D2);
            (A = west, C is C2 - 1, not(C < 0), R is R2, empty(R, C, S), D is D2);
            (A = east, C is C2 + 1, cols(CMAX), not(C >= CMAX), R is R2, empty(R, C, S), D is D2);
